...
const fiveYearsAgo = () => { const d = new Date(); d.setUTCFullYear(d.getUTCFullYear() - 5); return `${d.getUTCFullYear()}${String(d.getUTCMonth() + 1).padStart(2, "0")}${String(d.getUTCDate()).padStart(2, "0")}`; };
...
function trainOnce(state, card, result, mode) {
  ...
  state.journal.push({ key: card.key, venue: card.venue, race: card.r, combination: result.combination,
...
async function collectCurrent(state, maxRaces = 32) {
  const races = parseIndex(await fetchText(`${WT}/autorace/racecard/`)).slice(0, maxRaces);
...
async function discoverBackfill(state) {
  const cursor = state.collector.backfillVenueCursor % VENUE_SLUGS.length, slug = VENUE_SLUGS[cursor];
  state.collector.backfillVenueCursor = (cursor + 1) % VENUE_SLUGS.length;
  const found = parseIndex(await fetchText(`${WT}/autorace/${slug}/racecard/`, 8000)).filter(r => r.cup.slice(0, 8) >= fiveYearsAgo()).sort((a, b) => b.cup.localeCompare(a.cup));
  const known = new Set([...Object.keys(state.races), ...state.collector.backfillQueue.map(raceKey), ...state.journal.map(x => x.key)]);
  for (const race of found) if (!known.has(race.key)) { state.collector.backfillQueue.push(race); known.add(race.key); }
...
function prune(state) {
  const keep = new Set(state.journal.slice(-300).map(x => x.key)), cutoff = Date.now() - 14 * 86400_000;
  for (const [key, race] of Object.entries(state.races)) if (!keep.has(key) && Date.parse(race.fetchedAt || 0) < cutoff) delete state.races[key];
}

export async function updateState(state, mode = "scheduled") {
  const started = Date.now(), manual = mode === "current" || mode === "results";
  const beforeResults = Object.values(state.races).filter(r => r.result).length;
  state.collector.lastRun = new Date().toISOString(); state.collector.status = "running";
  try {
    await collectCurrent(state, manual ? 64 : 32);
    await collectPendingResults(state, mode === "results" ? 64 : 14);
    if (!manual) { await discoverBackfill(state); await runBackfill(state); }
    refreshOpenPredictions(state); prune(state);
    state.collector.status = "ok"; state.collector.lastSuccess = new Date().toISOString();
    ...
    state.collector.status = "partial"; state.collector.message = `一部取得失敗: ${String(error?.message || error).slice(0, 180)}`;
  }
  state.collector.durationMs = Date.now() - started;
  const afterResults = Object.values(state.races).filter(r => r.result).length;
  return { state, response: { ok: state.collector.status === "ok", mode, newResults: afterResults - beforeResults, resultsAvailable: afterResults, collector: state.collector, model: state.model } };
}
...
async function runCollect(env, mode = "scheduled") {
  const out = await updateState(await loadState(env.DB), mode);
  await saveState(env.DB, out.state);
  return out.response;
}

async function stateResponse(env) {
  return json(statePayload(await loadState(env.DB)));
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/.netlify/functions/state" || url.pathname === "/api/state") {
      try { return await stateResponse(env); } catch (e) { return json({ok:false,error:String(e?.message||e)},500); }
    }
    if (url.pathname === "/.netlify/functions/collect-and-train" || url.pathname === "/api/collect-and-train") {
      try { return json(await runCollect(env, url.searchParams.get("mode") || "current")); }
      catch (e) { return json({ok:false,error:String(e?.message||e)},500); }
    }
    return env.ASSETS.fetch(request);
  },
  async scheduled(controller, env, ctx) { ctx.waitUntil(runCollect(env, "scheduled")); }
};
